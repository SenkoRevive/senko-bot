...
config = {
    "name": "anime",
    "description": "✨ Угадай аниме по кадру"
}
...
async def command(
    ...
    view = GuessAnimeView(author=inter.author)
    await inter.response.send_message(embed=view.embed, view=view)
    view.message = await inter.original_message()


class GuessAnimeView(disnake.ui.View):
    def __init__(self, author: disnake.Member):
        self.author = author
        super().__init__(timeout=15.0)

        with open("data/anime.json", encoding="utf-8") as f:
